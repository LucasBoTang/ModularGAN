...
class Solver(object):
    ...
    def build_model(self):
        ...
        """
        # create modulars
        self.E = Encoder(conv_dim=self.e_conv_dim,
                         repeat_num=self.e_repeat_num)

        self.T = torch.nn.ModuleList()
        for c_dim in self.attr_dims:
            self.T.append(
                Transformer(
                    conv_dim=self.e_conv_dim * 4,
                    c_dim=c_dim,
                    repeat_num=self.t_repeat_num,
                ))
        ...
        self.g_optimizer = torch.optim.Adam(
            list(self.E.parameters()) + list(self.T.parameters()) +
            list(self.R.parameters()),
            self.g_lr,
    ...
    def restore_model(self, resume_epoch):
        ...
        """
        print(
            "Loading the trained models from step {}...".format(resume_epoch))
        E_path = os.path.join(self.model_save_dir,
                              "{}-E.ckpt".format(resume_epoch))
        T_path = os.path.join(self.model_save_dir,
                              "{}-T.ckpt".format(resume_epoch))
        R_path = os.path.join(self.model_save_dir,
                              "{}-R.ckpt".format(resume_epoch))
        D_path = os.path.join(self.model_save_dir,
                              "{}-D.ckpt".format(resume_epoch))
        self.E.load_state_dict(
            torch.load(E_path, map_location=lambda storage, loc: storage))
        self.T.load_state_dict(
            torch.load(T_path, map_location=lambda storage, loc: storage))
        self.R.load_state_dict(
            torch.load(R_path, map_location=lambda storage, loc: storage))
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.num_epochs + 1):
            ...
            for x_real, c_org_t in tbar:
                ...
                for j in range(self.num_transformer):
                    ...
                    d_loss_cls = F.binary_cross_entropy_with_logits(
                        out_cls, c_org_l_j, reduction="sum") / x_real.size(0)
                    ...
                    d_loss_j = (d_loss_real + d_loss_fake +
                                self.lambda_gp * d_loss_gp +
                                self.lambda_cls * d_loss_cls)

                    # logging
                    d_loss_dict["D/loss_src"] += d_loss_real.item(
                    ) + d_loss_fake.item()
                ...
                if i and i % self.n_critic == 0:
                    ...
                    for j in range(self.num_transformer):
                        ...
                        # compute generation loss and backward
                        g_loss_j = (g_loss_fake +
                                    self.lambda_cyc * g_loss_cyc +
            ...
            if (epoch % self.lr_update_step == 0
                    and epoch > self.num_epochs - self.num_epochs_decay):
